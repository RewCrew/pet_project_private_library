...
def test_get_all_books(books_service, client):
    ...
    expected = {'library':[]}
    result = client.simulate_get('/api/books/get_all_books',
                                 content_type = falcon.MEDIA_JSON)
    assert result.status_code == 200
    assert result.json == expected
...
def test_get_user_books(books_service, client, userbooks):
    ...
    result = client.simulate_get('/api/books/get_user_books',
                                 content_type = falcon.MEDIA_JSON,
                                 json = {'user_id':1})
...
def test_post_buy_book(books_service, client, userbooks_buy):
    ...
    result = client.simulate_post('/api/books/buy_book',
                                  content_type = falcon.MEDIA_JSON,
                                  json = {'user_id':1, 'book_isbn':123456781234})
    assert result.status_code == 200
    assert result.json == expected
...
def test_get_history_books(books_service, client, userbooks_find):
    ...
    expected = {'user 1 took and returned next books':
